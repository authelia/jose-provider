...
// NOTE: upstream's Ambig type, which exercises case-insensitive member name
// matching, is omitted here; this fork matches case-sensitively.
...
var unmarshalTests = []struct {
	...
	// NOTE: upstream's "only the most recent value is used" duplicate-key case is
	// omitted here; this fork rejects duplicate keys. See TestForkDuplicateKeys.
	...
	// NOTE: upstream's Ambig case-insensitive-matching case is omitted here; this
	// fork matches member names case-sensitively. See TestForkCaseSensitive.
	...
	{
		CaseName: Name(""),
		// NOTE: upstream repeats "I" here to test that a later null wins; this fork
		// rejects duplicate keys, so the duplicate is dropped from the input.
		in:  `{"I": null, "J": null}`,
		ptr: new(DoublePtr),
	...
	// additional tests for disallowUnknownFields
	{
		...
		// NOTE: upstream matches "x" to Embed0p's embedded image.Point.X
		// case-insensitively, making "extra" the first unknown field. This fork
		// matches case-sensitively, so "x" itself is the first unknown field.
		err:                   fmt.Errorf("json: unknown field \"x\""),
		disallowUnknownFields: true,
	},
	{
