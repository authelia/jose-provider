...
func NewEncrypter(enc ContentEncryption, rcpt Recipient, opts *EncrypterOptions) (Encrypter, error) {
	...
	if err := checkSuitableJWK(rcpt.Key, jwkUseEncryption, string(rcpt.Algorithm), jweKeyOps(rcpt.Algorithm, true)); err != nil {
		return nil, err
	}
...
func makeJWERecipient(alg KeyAlgorithm, encryptionKey any) (recipientKeyInfo, error) {
	if err := checkSuitableJWK(encryptionKey, jwkUseEncryption, string(alg), jweKeyOps(alg, true)); err != nil {
...
func (obj JSONWebEncryption) Decrypt(decryptionKey any) ([]byte, error) {
	...
	}, jwkUseEncryption, jweKeyOps(KeyAlgorithm(recipientHeaders.getString(headerAlgorithm)), false))
	if err != nil {
		return nil, err
	}
